...
describe("qa cli parsing", () => {
  it("parses stress seed and defaults run scope to qa", () => {
    expect(parseArgs(["run", "stress", "--seed", "123"])).toMatchObject({
      command: "run",
      scenario: "stress",
      scope: "qa",
      seed: 123,
  ...
  it("fails --trades with explicit TODO before needing a database", async () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});

    await expect(runCli(["run", "stress", "--trades", "500"], {})).resolves.toBe(1);
    expect(errorSpy).toHaveBeenCalledWith(
      "TODO: --trades requires exchange trade API before QA can stress trades",
    );
